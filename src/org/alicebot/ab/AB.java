...
public class AB {
  ...
  void findPatterns(Nodemapper node, String partialPatternThatTopic) {
    if (NodemapperOperator.isLeaf(node)) {
      ...
      if (node.category.getActivationCnt() > MagicNumbers.node_activation_cnt) {
        // log.info("LEAF: "+node.category.getActivationCnt()+".
        // "+partialPatternThatTopic+" "+node.shortCut); //Start writing to the
        // output stream
        leafPatternCnt++;
        try {
          ...
          if (!bot.brain.existsCategory(c) && !deletedGraph
              .existsCategory(c)/* && !unfinishedGraph.existsCategory(c) */) {
            patternGraph.addCategory(c);
  ...
  public ArrayList<Category> nonZeroActivationCount(ArrayList<Category> suggestedCategories) {
    ...
    for (Category c : suggestedCategories) {
      if (c.getActivationCnt() > 0)
        result.add(c);
      // else log.info("["+c.getActivationCnt()+"]
      // "+c.inputThatTopic());
    }
  ...
  public void terminalInteractionStep(Bot bot, String request, String textLine, Category c, String alicetemplate) {
    ...
    if (textLine.equals("q"))
      ...
      bot.writeQuit();
      /*
       * Nodemapper udcNode = bot.brain.findNode("*", "*", "*"); if (udcNode !=
       * null) { AIMLSet udcMatches = new AIMLSet("udcmatches");
       ...
       * AIMLSet("cmatches"); cMatches.addAll(cNode.category.getMatches());
       * cMatches.writeAIMLSet(); } if (passed.size() > 0) { AIMLSet difference
       * = new AIMLSet("difference"); AIMLSet intersection = new
       * AIMLSet("intersection"); for (String s : passed) if
       ...
       */
      System.exit(0);
    } else if (textLine.equals("skip") || textLine.equals("")) { // skip this
                                                                 // one for now
      skipCategory(c);
